...
export default function PersonalTransactionsPage() {
  ...
  const filteredItems = useMemo(() => {
    let result = items;
    if (filterYear !== "all") result = result.filter((t) => new Date(t.date).getFullYear() === Number(filterYear));
    if (filterMonth !== "all") result = result.filter((t) => new Date(t.date).getMonth() === Number(filterMonth));
    return result;
